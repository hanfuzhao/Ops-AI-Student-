...
# The employee database, policy docs, and expense rules ship with week 5, so
# reuse them instead of duplicating the 49MB database. The access policy is
# the new week 6 file.
DATA_DIR = Path(__file__).resolve().parents[2] / "week5" / "data"
ACCESS_POLICY = Path(__file__).resolve().parents[1] / "data" / "access_control.json"
...
class Agent:
    ...
    def __init__(self, db_path: str, api_key: str = None,
        ...
        # Week 6 guardrails: who can see what, how fast they can ask, and how
        # much they can spend.
        self.access_controller = AccessController(str(ACCESS_POLICY))
    ...
    def query(self, user_query: str, user_id: str = "anon",
              user_role: str = "engineer") -> Dict[str, Any]:
        logger.info("query (%s/%s): %s", user_id, user_role, user_query)

        # Guardrails run before the LLM so a blocked request costs nothing.
        if not self.rate_limiter.is_allowed(user_id):
            self.access_controller.log_access(user_role, "query", allowed=False)
            return {"answer": "Rate limit exceeded. Try again in a minute.",
                    "error": "rate_limit", "tokens_used": 0, "cost": 0.0,
                    "role": user_role}

        estimated_cost = 0.01
        if not self.cost_enforcer.can_afford_query(user_id, estimated_cost, user_role):
            self.access_controller.log_access(user_role, "query", allowed=False)
            return {"answer": "Budget exceeded for this role.",
                    "error": "budget", "tokens_used": 0, "cost": 0.0,
                    "role": user_role}
        ...
        try:
            ...
            # Strip any sensitive values this role isn't cleared for, then
            # charge the query against their budget.
            answer = self.access_controller.redact_response(
                user_role, answer or "(no answer returned)")
